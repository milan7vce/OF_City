...
{
    m(*this, *this); //Field<Type>::autoMap(m);
    m(refValue_, refValue_); //refValue_.autoMap(m);
    m(valueFraction_, valueFraction_); //valueFraction_.autoMap(m);
}
...
void mixedFixedValueSlipFvPatchField<Type>::write(Ostream& os) const
{
    ...
    writeEntry(os, "refValue", refValue_); //refValue_.writeEntry("refValue", os);
    writeEntry(os, "valueFraction", valueFraction_); //valueFraction_.writeEntry("valueFraction", os);
}
